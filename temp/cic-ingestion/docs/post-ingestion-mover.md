...
- If a processed copy is missing but an original exists in an inbox folder, the mover will move the original into processed (idempotent).
- If both processed and original exist, the original is moved to `_Archive/.../originals/` to avoid inbox duplication.
